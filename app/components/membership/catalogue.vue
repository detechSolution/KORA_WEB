...
const membershipPeriods = computed(() => [
  "MONTHLY",
  "QUARTERLY",
  "YEARLY",
  ...(props.gifting ? [] : ["CUSTOM"]),
  "PASSES",
]);
...
function openPassModal(pass: Passes) {
  if (props.gifting) {
    emit("giftPass", pass);
    return;
  }
...
function openMembershipModal(tier: any) {
  if (props.gifting) {
    if (tier.selectedOption)
      emit("giftMembership", tier, tier.selectedOption);
    return;
  }
  if (authStore.isAuthenticated) {
...
<template>
  <div
    class="relative bg-background dark:bg-secondary-900 text-foreground dark:text-white transition-colors duration-300 w-full overflow-hidden"
    :class="gifting ? 'gift-catalogue' : 'min-h-screen'"
  >
    <div
      v-if="!gifting"
    ...
    <MembershipBookingModal
      v-if="!gifting && selectedMembershipTier"
